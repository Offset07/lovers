...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")
        await self.highrise.walk_to(Position(16.5 , 0.5 , 1.5 , "FrontLeft"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala")   
        await self.highrise.chat(f"Bienvenue {user.username} dans ❤️TROUVER VOTRE AMOUR❤️.Les danses sont de 1 à 95.")
